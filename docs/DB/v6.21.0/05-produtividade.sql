...
BEGIN
  SELECT FUNC_CHECK_SCRIPT('xx-produtividade.sql', '') INTO CHECK_RESULT FROM DUAL ;
  IF (CHECK_RESULT <> 'TRUE') THEN 
    DBMS_OUTPUT.PUT_LINE(CHECK_RESULT);
  ELSE 
        INSERT INTO VERSAO (DTH, NUMERO_VERSAO, SCRIPT) VALUES (SYSDATE, '6.21.0','05-produtividade.sql');
/************************************************************************
**        COLOQUE O SCRIPT À SER EXECUTADO ENTRE ESTA DEMARCAÇÃO       **
************************************************************************/

      EXECUTE IMMEDIATE 'ALTER TABLE EQUIPE_CARREGAMENTO_EXPEDICAO RENAME COLUMN DTH_VINCULO TO DTH_INICIO';
      EXECUTE IMMEDIATE 'ALTER TABLE EQUIPE_CARREGAMENTO_EXPEDICAO ADD DTH_FINAL DATE';
...
END;
 ...
 --PROCEDURE DE PRODUTIVIDADE DETALHADA CONSIDERANDO OS CAMPOS DE DATA INICIO E DATA FIM NO CARREGAMENTO DE EXPEDICAO
...
SELECT  'CONF. RECEBIMENTO' as DSC_ATIVIDADE,
		...
		COUNT(DISTINCT RC.COD_PRODUTO || '/' || RC.DSC_GRADE) as QTD_PRODUTOS,
		SUM(PROD.NUM_VOLUMES * RC.QTD_CONFERIDA) as QTD_VOLUMES,
 ...
 WHERE TO_DATE(TO_CHAR(OS.DTH_INICIO_ATIVIDADE,'DD/MM/YYYY'),'DD/MM/YYYY') BETWEEN TO_DATE(DTH_INICIO_PARAM,'DD/MM/YYYY') AND TO_DATE(DTH_FIM_PARAM,'DD/MM/YYYY')
   AND ((RC.QTD_DIVERGENCIA = 0 AND RC.IND_DIVERG_VOLUMES = 'N' AND RC.IND_DIVERGENCIA_PESO = 'N') OR RC.COD_NOTA_FISCAL IS NOT NULL)
	...
	SELECT
		...
		CAST(SUM(RC.QTD_CONFERIDA/QTD_PES.QTD_USUARIOS)as NUMBER(30,2)) as QTD_PRODUTOS,
		CAST(SUM((RC.QTD_CONFERIDA * P.NUM_VOLUMES)/QTD_PES.QTD_USUARIOS) as NUMBER(20,2)) as QTD_VOLUMES,
    ...
    LEFT JOIN (SELECT COD_RECEBIMENTO, COUNT(COD_USUARIO) as QTD_USUARIOS FROM RECEBIMENTO_DESCARGA GROUP BY COD_RECEBIMENTO) QTD_PES
      ON QTD_PES.COD_RECEBIMENTO = RD.COD_RECEBIMENTO
    LEFT JOIN RECEBIMENTO_CONFERENCIA RC ON RC.COD_RECEBIMENTO = RD.COD_RECEBIMENTO
                                        AND ((RC.QTD_DIVERGENCIA = 0 AND RC.IND_DIVERG_VOLUMES = 'N' AND RC.IND_DIVERGENCIA_PESO = 'N') OR RC.COD_NOTA_FISCAL IS NOT NULL)
   ...
   WHERE TO_DATE(TO_CHAR(RD.DTH_VINCULO,'DD/MM/YYYY'),'DD/MM/YYYY') BETWEEN TO_DATE(DTH_INICIO_PARAM,'DD/MM/YYYY') AND TO_DATE(DTH_FIM_PARAM,'DD/MM/YYYY')
	...
	FROM
		...
		WHERE TO_DATE(TO_CHAR(MSC.DTH_CONFERENCIA,'DD/MM/YYYY'),'DD/MM/YYYY') BETWEEN TO_DATE(DTH_INICIO_PARAM,'DD/MM/YYYY') AND TO_DATE(DTH_FIM_PARAM,'DD/MM/YYYY')
	GROUP BY
		OS.COD_PESSOA,
