INSERT INTO VERSAO (DTH, NUMERO_VERSAO, SCRIPT) VALUES (SYSDATE, '5.0.0','correcao_view.sql');
...
SELECT SUM(RE.QTD_CONFERIDA * PE.QTD_EMBALAGEM) AS QTD,
 ...
 INNER JOIN (SELECT DISTINCT CASE WHEN DTH_FINAL_ATIVIDADE IS NULL THEN TO_DATE('31/12/9999','dd/mm/yyyy') ELSE DTH_FINAL_ATIVIDADE END AS DTH_FINAL_ATIVIDADE,
                    OS.COD_OS,
                    OS.COD_RECEBIMENTO,
                    PE.COD_PRODUTO,
                    PE.DSC_GRADE,
                    RANK() OVER(PARTITION BY RE.COD_RECEBIMENTO, PE.COD_PRODUTO, PE.DSC_GRADE ORDER BY DTH_FINAL_ATIVIDADE DESC) RANK
                  FROM RECEBIMENTO_EMBALAGEM RE
                 INNER JOIN PRODUTO_EMBALAGEM PE ON PE.COD_PRODUTO_EMBALAGEM = RE.COD_PRODUTO_EMBALAGEM
                 INNER JOIN ORDEM_SERVICO OS ON OS.COD_OS = RE.COD_OS) OS
    ...
    PE.DSC_GRADE,
    OS.COD_OS,
    RE.COD_NORMA_PALETIZACAO) RE ON RE.COD_RECEBIMENTO = R.COD_RECEBIMENTO;
