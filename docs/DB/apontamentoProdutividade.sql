...
BEGIN
  ...
  DELETE FROM APONTAMENTO_PRODUTIVIDADE WHERE DTH_ATIVIDADE >= DTH_INICIAL AND DTH_ATIVIDADE <= DTH_FINAL;
...
SELECT DSC_ATIVIDADE, COD_PESSOA,DTH_ATIVIDADE , QTD_PRODUTOS, QTD_VOLUMES, QTD_CUBAGEM, QTD_PESO, QTD_PALETES
  FROM (
SELECT 'CONF. RECEBIMENTO' as DSC_ATIVIDADE,
       P.COD_PESSOA,
  ...
  LEFT JOIN SUM_PESO_PRODUTO PP ON PP.COD_PRODUTO = V.COD_PRODUTO AND V.DSC_GRADE = V.DSC_GRADE
...
UNION
SELECT 'EQUIPE DE DESCARGA' as DSC_ATIVIDADE,
  ...
  LEFT JOIN SUM_PESO_PRODUTO PP ON PP.COD_PRODUTO = V.COD_PRODUTO AND PP.DSC_GRADE = V.DSC_GRADE
  GROUP BY RD.COD_USUARIO, TO_DATE(RD.DTH_VINCULO)
UNION
SELECT 'SEPARACAO' as DSC_ATIVIDADE,
        A.COD_USUARIO as COD_PESSOA,
        TO_DATE(TRIM(SUBSTR(DTH_CONFERENCIA,1,8))) as DTH_ATIVIDADE,
 ...
 INNER JOIN APONTAMENTO_SEPARACAO_MAPA A ON A.COD_MAPA_SEPARACAO = M.COD_MAPA_SEPARACAO
 LEFT JOIN SUM_PESO_PRODUTO SPP ON SPP.COD_PRODUTO = M.COD_PRODUTO AND SPP.DSC_GRADE = M.DSC_GRADE AND M.COD_PRODUTO_EMBALAGEM IS NOT NULL
 LEFT JOIN PRODUTO_VOLUME PV ON PV.COD_PRODUTO_VOLUME = M.COD_PRODUTO_VOLUME
 WHERE M.IND_CONFERIDO = 'S'
 GROUP BY A.COD_USUARIO,
          TO_DATE(TRIM(SUBSTR(DTH_CONFERENCIA,1,8)))
UNION
  ...
  LEFT JOIN SUM_PESO_PRODUTO SPP ON MSC.COD_PRODUTO = SPP.COD_PRODUTO AND MSC.DSC_GRADE = SPP.DSC_GRADE
