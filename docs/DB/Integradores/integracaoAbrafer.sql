...
VALUES (25,1,
 ...
 WHERE NFS.NOPPRO NOT IN ('||'''5929'''||','||'''6929'''||','||'''1202'''||','||'''2202'''||') AND 
       ...
       NFS.CODEMP = 1   AND 
       NFS.CODFIL = 2   AND
       (CONVERT(DATETIME,CONVERT(VARCHAR(10),NFS.DATEMI,120) + '||''' '''||' + (CAST (NFS.HOREMI / 60 AS VARCHAR) + '||''':'''||' + CAST (NFS.HOREMI - ((NFS.HOREMI / 60) * 60) AS VARCHAR))) > CONVERT(DATETIME,'||'''2017-09-27 09:14:43'''||'))
