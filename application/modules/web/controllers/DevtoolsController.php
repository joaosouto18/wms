...
class Web_DevtoolsController extends \Wms\Controller\Action
{
    ...
    public function indexAction()
    {
        ...
        $settings['language'] = 'pt'; // Refer to the lang/ folder for supported languages
...
// Compress content, can be turned off to view error messages in browser
        ...
        if (isset($_SERVER['LINFO_NCURSES']) && php_sapi_name() == 'cli') {
            $output = new Core\Linfo\Output\Ncurses($linfo);
        }
        else {
            switch (array_key_exists('out', $_GET) ? strtolower($_GET['out']) : 'html') {
                ...
                case 'jsonp': // To use JSON-P, pass the GET arg - callback=function_name
                    $output = new Core\Linfo\Output\Json($linfo, array_key_exists('callback', $_GET) ? $_GET['callback'] : null);
                    break;

                case 'php_array':
                    $output = new Core\Linfo\Output\Serialized($linfo);
                    break;

                case 'xml':
                    $output = new Core\Linfo\Output\Xml($linfo);
                    break;
            }
        }
