...
class Expedicao_EtiquetaController extends Action
{
    ...
    public function imprimirAction()
    {
        ...
        if ($ExpedicaoRepo->getQtdMapasPendentesImpressao($idExpedicao) > 0)
            $linkMapa = '<a href="' . $this->view->url(array('controller' => 'etiqueta', 'action' => 'gerar-pdf-ajax', 'id' => $idExpedicao, 'tipo' => 'mapa', 'central' => $central, 'cargas' =>$cargas)) . '" target="_blank" ><img style="vertical-align: middle" src="' . $this->view->baseUrl('img/icons/page_white_acrobat.png') . '" alt="#" /> Mapa de Separação</a>';
    ...
    public function gerarPdfAjaxAction()
    {
        ...
        if ($tipo == "mapa") {
            if ($ExpedicaoRepo->getQtdMapasPendentesImpressao($idExpedicao) > 0) {
                ...
                $mapa->setCargasSelecionadas($cargas);
