...
templates.push({
    ...
    template:
        ...
        '}' +
        '</style>' +
        '<div id="wms-dialog-modal" ng-controller="detalhamentoRastreioCtrl" ng-init="prepare()" style="font-size: 12px;" class="ui-dialog-content ui-widget-content">' +
        '<form id="mainForm" enctype="application/x-www-form-urlencoded" accept-charset="UTF-8" action="">' +
        '<div>' +
        '<fieldset id="fieldset-Buscar">' +
        '<legend>Expedição rastreada: {{expedicao.codExpedicao}}</legend>' +
        '<div class="field" style="margin: 0; padding-top: 10px">' +
        '<table class="table-info-grid">' +
        '<thead>' +
        '<tr>' +
        ...
        '<tbody>' +
        '<tr>' +
        '<td>{{expedicao.codCarga}}</td>' +
        '<td>{{expedicao.nomCliente}}</td>' +
        '<td>{{expedicao.dthInicio}}</td>' +
        '<td>{{expedicao.dthFim}}</td>' +
        '</tr>' +
        '</tbody>' +
        '</table>' +
        '</div>' +
        ...
        '</div>' +
        '</div>' +
        '<table class="gTable" style="width:956px!important">' +
        '<tbody >' +
        '<tr class="gTTitle">' +
        '<td ng-repeat="column in gridColumns" width="{{column.width}}">' +
        '<div><a href="" title="" class="sort" ng-click="ordenarPor(column.name)"><span>{{column.label}}</span></a></div>' +
        ...
        '</tr>' +
        '<tr>' +
        '<td colspan="100%" align="center" ng-show="noResults" >Nenhum resultado</td>' +
        '</tr>' +
        '</table>' +
        '</div>' +
        '</div>' +
