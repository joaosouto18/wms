    ...
    private function prepareDBALConfiguration(array $config = array())
    {
        ...
        if (isset($dbalConfig['connections'])) {
            ...
            foreach ($configConnections as $name => $connection) {
			  $defaultConnectionTmp = $defaultConnection;
			  if( 'oci8' == $connection['parameters']['driver'] ) {
				unset($defaultConnectionTmp['parameters']['host']);
			  }
			  $name = isset($connection['id']) ? $connection['id'] : $name;
			  $connections[$name] = array_replace_recursive($defaultConnectionTmp, $connection);
            }
        } else {
            $connections = array(
