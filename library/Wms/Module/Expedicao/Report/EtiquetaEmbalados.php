...
class EtiquetaEmbalados extends eFPDF
{
    ...
    public function imprimirExpedicaoModelo($volumePatrimonio, $mapaSeparacaoEmbaladoRepo, $modeloEtiqueta, $fechaEmbaladosNoFinal = false)
    {
        ...
        switch ($modeloEtiqueta) {
            ...
            case 6:
                //LAYOUT PLANETA
                self::bodyExpedicaoModelo6($volumePatrimonio, $mapaSeparacaoEmbaladoRepo, $fechaEmbaladosNoFinal);
                break;
            case 7:
                //LAYOUT MBLED
    ...
    private function bodyExpedicaoModelo6($volumes, $mapaSeparacaoEmbaladoRepo, $fechaEmbaladosNoFinal)
    {
        foreach ($volumes as $volume) {
            ...
            if ($fechaEmbaladosNoFinal)
                $impressao = $volume['NUM_SEQUENCIA'].'/'.$totalEtiquetas;
            else if ($existeItensPendentes == false)
    ...
    private function bodyExpedicaoModelo7($volumes)
    {
        ...
        foreach ($volumes as $volume) {
            ...
            $this->MultiCell(70, 5, 'TRANSP.: '.$impressao, 0, 'L');
