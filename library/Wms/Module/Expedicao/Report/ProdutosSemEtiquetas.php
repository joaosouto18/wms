...
class ProdutosSemEtiquetas extends Pdf
{
    ...
    public function Header()
    {
        ...
        $this->Cell(20, 10, utf8_decode("RELATÓRIO DE PRODUTOS SEM ETIQUETAS - EXPEDIÇÃO ". $this->idExpedicao), 0, 1);
    ...
    public function imprimir($idExpedicao, $produtos)
    {
        ...
        $this->Cell(135, 5, utf8_decode("Descrição"), "TB");
        $this->Cell(35, 5, "Grade", "TB");
        $this->Cell(35, 5, "Qtde.", "TB");
        $this->Ln();

        /** @var \Wms\Domain\Entity\Produto $produto */
        foreach($produtos as $key => $produto) {
            $this->Cell(15, 5, $produto['Codigo'], 0);
            $this->Cell(135, 5, $produto['Produto'], 0);
            $this->Cell(35, 5, $produto['Grade'], 0);
            $this->Cell(35, 5, $produto['Quantidade'], 0);

            $this->Ln();
        }

        $this->Output('Produtos-Sem_Etiquetas-'.$idExpedicao.'.pdf','D');
    }
}
