...
class ResultadoComparativoInventarioForm extends Form
{
    ...
    public function setDefaultsGrid($values) {
        ...
        if ((count($values['apenas-wms']) >0)) {
            $this->getSubForm('apenas-wms')->setDefaultsGrid($values['apenas-wms'], 'Estes produtos foram contados no WMS mas não estão presentes no ERP.');
        }
        if ((count($values['apenas-erp']) >0)) {
            $this->getSubForm('apenas-erp')->setDefaultsGrid($values['apenas-erp'],'Estes produtos estão apenas no inventário do ERP e não foram contados no WMS.');
        }
    }
