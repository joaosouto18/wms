...
class InventarioRepository extends EntityRepository
{
    ...
    public function verificaReservas($idInventario)
    {
        $source = $this->_em->createQueryBuilder()
            ...
            CASE WHEN exp.id IS NOT NULL THEN 'Expedição Código:' || exp.id
                 WHEN ressup.id IS NOT NULL THEN 'Ressuprimento OS:' || ressup.id
                 WHEN palete.id IS NOT NULL THEN 'Palete :' || palete.id
