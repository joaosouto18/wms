...
//                   ORDER BY HIST.DTH_ESTOQUE, DE.NUM_RUA";
    ...
    public function getEnderecosPorProduto($codbarras) {
        ...
        $SQL = "SELECT 
                    END.ENDERECO,
                    END.TIPO,
                    NVL(END.QTD, RES.QTD) AS QTD,
                    END.COD_PRODUTO,
                    END.DSC_GRADE,
                    END.DSC_PRODUTO,
                    END.PICKING_PRODUTO,
                    END.COD_BARRAS,
                    NVL(RES.QTD_RESERVADA, END.QTD_RESERVADA) AS QTD_RESERVADA,
                    NVL(RES.RESERVAS, END.RESERVAS) AS RESERVAS
                FROM (
                    SELECT DISTINCT
                    ...
                    WHERE PE.COD_BARRAS = '$codbarras' OR PV.COD_BARRAS = '$codbarras') END
                LEFT JOIN (
                    SELECT
                         DE.DSC_DEPOSITO_ENDERECO as ENDERECO,
                         ...
                         P.DSC_GRADE,
                         P.DSC_PRODUTO,
                         DE.DSC_DEPOSITO_ENDERECO AS PICKING_PRODUTO,
                         PE.COD_BARRAS,
                         SUM(REP.QTD_RESERVADA) QTD_RESERVADA,
                         LISTAGG (RE.COD_RESERVA_ESTOQUE,',') WITHIN GROUP (ORDER BY RE.COD_RESERVA_ESTOQUE) RESERVAS
                    ...
                    LEFT JOIN PRODUTO_EMBALAGEM PE ON PE.COD_PRODUTO = REP.COD_PRODUTO AND PE.DSC_GRADE = REP.DSC_GRADE
                    ...
                    INNER JOIN DEPOSITO_ENDERECO DE ON DE.COD_DEPOSITO_ENDERECO = RE.COD_DEPOSITO_ENDERECO
                      AND (DE.COD_DEPOSITO_ENDERECO = PE.COD_DEPOSITO_ENDERECO OR DE.COD_DEPOSITO_ENDERECO = PV.COD_DEPOSITO_ENDERECO)
                    INNER JOIN CARACTERISTICA_ENDERECO CE ON CE.COD_CARACTERISTICA_ENDERECO = DE.COD_CARACTERISTICA_ENDERECO
                    WHERE RE.IND_ATENDIDA = 'N' AND RE.TIPO_RESERVA = 'E' AND (PE.COD_BARRAS = '$codbarras' OR PV.COD_BARRAS = '$codbarras')
                    GROUP BY DE.DSC_DEPOSITO_ENDERECO,CE.DSC_CARACTERISTICA_ENDERECO, NULL, P.COD_PRODUTO, P.DSC_GRADE, P.DSC_PRODUTO, DE.DSC_DEPOSITO_ENDERECO, PE.COD_BARRAS
                ) RES ON RES.COD_PRODUTO = END.COD_PRODUTO AND RES.DSC_GRADE = END.DSC_GRADE AND RES.PICKING_PRODUTO = END.ENDERECO";
