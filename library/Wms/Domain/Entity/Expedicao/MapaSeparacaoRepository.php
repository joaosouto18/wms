...
//        WHERE MSP.QTD_SEPARAR = NVL(MSC.QTD_CONFERIDA,0)
    ...
    public function getMapaPendenteSeparacao($codExpedicao = null, $codMapaSeparacao = null)
    {
        ...
        $sql = "SELECT MS.COD_MAPA_SEPARACAO,
                    ...
                    CASE WHEN SMS.TOTAL_SEPARADO = 0 OR MSP.QTD_SEPARAR = 0 THEN '0%' ELSE CAST((SMS.TOTAL_SEPARADO/MSP.QTD_SEPARAR) * 100 as NUMBER(6,2)) || '%' END PERCENTUAL_SEPARACAO,
                    MS.COD_EXPEDICAO,
                    CASE WHEN SMS.TOTAL_SEPARADO = 0 OR MSP.QTD_SEPARAR = 0 THEN 0 ELSE NVL(CAST((SMS.TOTAL_SEPARADO/MSP.QTD_SEPARAR) * 100 as NUMBER(6,2)),0) END PERCENTUAL
