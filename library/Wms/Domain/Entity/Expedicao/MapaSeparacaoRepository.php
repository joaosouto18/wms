...
class MapaSeparacaoRepository extends EntityRepository
{
    ...
    public function getResumoConferenciaMapaByExpedicao ($idExpedicao){
        $SQL = "SELECT MS.COD_MAPA_SEPARACAO, MS.DTH_CRIACAO, TRIM(MS.DSC_QUEBRA) QUEBRA, NVL(SUM(MSP.QTD_TOTAL),0) QTD_TOTAL,
                ...
                LEFT JOIN (
                  SELECT SUM(MSP.QTD_SEPARAR * MSP.QTD_EMBALAGEM) QTD_TOTAL, MS.COD_MAPA_SEPARACAO, MSP.COD_MAPA_SEPARACAO_PRODUTO, MSP.IND_CONFERIDO
                  FROM MAPA_SEPARACAO_PRODUTO MSP
    ...
    {
        $sql = "SELECT SUM(NVL(MSP.QTD_SEPARAR * MSP.QTD_EMBALAGEM, 0)) - NVL(MSC.QTD_CONFERIDA, 0) AS QTD_PRODUTO_CONFERIR, SUM(NVL(MSP.QTD_SEPARAR * MSP.QTD_EMBALAGEM, 0)), NVL(MSC.QTD_CONFERIDA, 0), MSP.COD_PRODUTO
                FROM MAPA_SEPARACAO MS
                INNER JOIN MAPA_SEPARACAO_PRODUTO MSP ON MSP.COD_MAPA_SEPARACAO = MS.COD_MAPA_SEPARACAO
                LEFT JOIN (
    ...
    {
        $sql = "SELECT PESSOA.COD_PESSOA, PESSOA.NOM_PESSOA, MSP.COD_PRODUTO, MSP.NUM_CAIXA_PC_INI, MSP.NUM_CAIXA_PC_FIM
                  FROM MAPA_SEPARACAO_PRODUTO MSP
                  INNER JOIN MAPA_SEPARACAO MS ON MSP.COD_MAPA_SEPARACAO = MS.COD_MAPA_SEPARACAO
                  INNER JOIN (
                  ...
                  INNER JOIN (
                    SELECT MSE.COD_MAPA_SEPARACAO, MSE.COD_PESSOA, MSE.COD_MAPA_SEPARACAO_EMB_CLIENTE, MSC.COD_PRODUTO, MSC.DSC_GRADE
                    FROM MAPA_SEPARACAO MS
                    INNER JOIN MAPA_SEPARACAO_EMB_CLIENTE MSE ON MSE.COD_MAPA_SEPARACAO = MS.COD_MAPA_SEPARACAO
                    INNER JOIN MAPA_SEPARACAO_CONFERENCIA MSC ON MS.COD_MAPA_SEPARACAO = MSC.COD_MAPA_SEPARACAO AND MSE.COD_MAPA_SEPARACAO_EMB_CLIENTE = MSC.COD_MAPA_SEPARACAO_EMBALADO
                    WHERE MS.COD_MAPA_SEPARACAO = $idMapaSeparacao
                    GROUP BY MSE.COD_MAPA_SEPARACAO, MSE.COD_PESSOA, MSE.COD_MAPA_SEPARACAO_EMB_CLIENTE, MSC.COD_PRODUTO, MSC.DSC_GRADE
                  ) MSE ON PESSOA.COD_PESSOA = MSE.COD_PESSOA AND MS.COD_MAPA_SEPARACAO = MSE.COD_MAPA_SEPARACAO AND MSC.COD_PRODUTO = MSE.COD_PRODUTO
              WHERE MS.COD_MAPA_SEPARACAO = $idMapaSeparacao
              GROUP BY MSC.QTD_CONFERIDA, PESSOA.COD_PESSOA, PESSOA.NOM_PESSOA, MSP.COD_PRODUTO, MSP.NUM_CAIXA_PC_INI, MSP.NUM_CAIXA_PC_FIM
              HAVING SUM(MSP.QTD_EMBALAGEM * MSP.QTD_SEPARAR - NVL(MSP.QTD_CORTADO,0)) - SUM(NVL(MSC.QTD_CONFERIDA,0)) > 0
              ORDER BY MSP.NUM_CAIXA_PC_INI, MSP.NUM_CAIXA_PC_FIM";
