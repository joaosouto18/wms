    ...
    public function getResumoConferenciaMapaByExpedicao ($idExpedicao){
        $SQL = "SELECT MS.COD_MAPA_SEPARACAO, MS.DTH_CRIACAO, TRIM(MS.DSC_QUEBRA) QUEBRA, NVL(SUM(MSP.QTD_TOTAL),0) QTD_TOTAL,
                ...
                (CASE WHEN MSP.IND_CONFERIDO = 'S'
                  THEN '100%'
                  ELSE
                    (CASE WHEN NVL(SUM(MSC.QTD_CONFERIDA),0) * 100 / NVL(SUM(MSP.QTD_TOTAL),0) > 100
                    THEN '100%'
                    ELSE
                      CAST(NVL(SUM(MSC.QTD_CONFERIDA),0) * 100 / NVL(SUM(MSP.QTD_TOTAL),0) AS NUMBER(6,2)) || '%' END) END) AS PERCENTUAL,
                      MS.COD_EXPEDICAO
