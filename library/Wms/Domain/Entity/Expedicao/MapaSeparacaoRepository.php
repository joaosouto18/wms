    ...
    public function getClientesByConferencia($idMapaSeparacao)
    {
        ...
        $sql = "SELECT P.NOM_PESSOA, P.COD_PESSOA,
                LISTAGG(MSPROD.NUM_CAIXA_PC_INI, ',') WITHIN GROUP (ORDER BY MSPROD.NUM_CAIXA_PC_INI) AS NUM_CAIXA_PC_INI
                    ...
                    INNER JOIN PRODUTO PROD ON PROD.COD_PRODUTO = PP.COD_PRODUTO AND PROD.DSC_GRADE = PP.DSC_GRADE
                    LEFT JOIN (
                      SELECT SUM(MSC.QTD_EMBALAGEM * MSC.QTD_CONFERIDA) QTD_CONFERIDA, MSC.COD_PRODUTO, MSC.DSC_GRADE, MS.COD_MAPA_SEPARACAO, MSC.COD_PESSOA, MSC.COD_MAPA_SEPARACAO_EMBALADO
                      FROM MAPA_SEPARACAO_CONFERENCIA MSC
                      INNER JOIN MAPA_SEPARACAO MS ON MSC.COD_MAPA_SEPARACAO = MS.COD_MAPA_SEPARACAO
                      WHERE MSC.COD_MAPA_SEPARACAO = $idMapaSeparacao
                      GROUP BY MSC.COD_PRODUTO, MSC.DSC_GRADE, MS.COD_MAPA_SEPARACAO, MSC.COD_PESSOA, MSC.COD_MAPA_SEPARACAO_EMBALADO ) MSC ON MSC.COD_MAPA_SEPARACAO = MS.COD_MAPA_SEPARACAO AND MSC.COD_PRODUTO = PROD.COD_PRODUTO AND MSC.DSC_GRADE = PROD.DSC_GRADE AND MSC.COD_PESSOA = P.COD_PESSOA
                LEFT JOIN MAPA_SEPARACAO_EMB_CLIENTE MSE ON MSE.COD_MAPA_SEPARACAO_EMB_CLIENTE = MSC.COD_MAPA_SEPARACAO_EMBALADO
                WHERE MS.COD_MAPA_SEPARACAO = $idMapaSeparacao AND (MSPROD.QTD_SEPARAR > NVL(MSC.QTD_CONFERIDA,0) OR MSE.COD_STATUS = $statusEmbalado)
