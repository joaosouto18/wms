...
class MapaSeparacaoConferenciaRepository extends EntityRepository
{
    ...
    {
        $sql = "SELECT MS.COD_MAPA_SEPARACAO, SUM((MSP.QTD_EMBALAGEM * MSP.QTD_SEPARAR) - MSP.QTD_CORTADO) - NVL(SUM(MSC.QTD_EMBALAGEM * MSC.QTD_CONFERIDA),0) QTD_CONFERIR,
                    MSP.COD_PRODUTO_EMBALAGEM, MSP.COD_PRODUTO_VOLUME
                    FROM MAPA_SEPARACAO MS
                    INNER JOIN MAPA_SEPARACAO_PRODUTO MSP ON MSP.COD_MAPA_SEPARACAO = MS.COD_MAPA_SEPARACAO
                    LEFT JOIN MAPA_SEPARACAO_CONFERENCIA MSC ON MSC.COD_MAPA_SEPARACAO = MS.COD_MAPA_SEPARACAO AND MSP.COD_PRODUTO = MSC.COD_PRODUTO AND MSP.DSC_GRADE = MSC.DSC_GRADE
                    WHERE MS.COD_EXPEDICAO = $idExpedicao 
                    GROUP BY MS.COD_MAPA_SEPARACAO, MSP.COD_PRODUTO, MSP.DSC_GRADE, MSP.COD_PRODUTO_EMBALAGEM, MSP.COD_PRODUTO_VOLUME
                    HAVING (SUM((MSP.QTD_EMBALAGEM * MSP.QTD_SEPARAR) - MSP.QTD_CORTADO) - NVL(SUM(MSC.QTD_EMBALAGEM * MSC.QTD_CONFERIDA),0) > 0)";
        return $this->getEntityManager()->getConnection()->query($sql)->fetchAll(\PDO::FETCH_ASSOC);
    }
