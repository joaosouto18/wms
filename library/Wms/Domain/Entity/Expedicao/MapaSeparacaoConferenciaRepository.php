...
class MapaSeparacaoConferenciaRepository extends EntityRepository
{
    ...
    public function getProdutosClientesByExpedicao($idExpedicao,$idLinhaSeparacao)
    {
        ...
        $sql = "SELECT SUM(CONF.QTD_CONFERIDA * CONF.QTD_EMBALAGEM) QUANTIDADE_CONFERIDA, PP.SEQUENCIA, C.COD_CARGA_EXTERNO, PROD.COD_PRODUTO, PROD.DSC_GRADE, PROD.DSC_PRODUTO, LS.DSC_LINHA_SEPARACAO, E.DTH_INICIO, PP.DSC_PLACA_CARGA, P.NOM_PESSOA, NVL(PF.NUM_CPF, PJ.NUM_CNPJ) CPF_CNPJ, PE.DSC_ENDERECO, PE.NOM_BAIRRO, PE.NOM_LOCALIDADE, S.COD_REFERENCIA_SIGLA, PP.COD_PEDIDO
                    FROM MAPA_SEPARACAO_CONFERENCIA CONF
                    INNER JOIN MAPA_SEPARACAO MS ON MS.COD_MAPA_SEPARACAO = CONF.COD_MAPA_SEPARACAO
                    INNER JOIN EXPEDICAO E ON MS.COD_EXPEDICAO = E.COD_EXPEDICAO
                    LEFT JOIN (SELECT E.COD_EXPEDICAO, C.DSC_PLACA_CARGA, C.COD_CARGA, PP.COD_PRODUTO, PP.DSC_GRADE, P.SEQUENCIA, SUM(PP.QUANTIDADE - NVL(PP.QTD_CORTADA,0)) QUANTIDADE_CONFERIDA, P.COD_PESSOA, P.COD_PEDIDO
                                FROM EXPEDICAO E
                                INNER JOIN CARGA C ON E.COD_EXPEDICAO = C.COD_EXPEDICAO
                                INNER JOIN PEDIDO P ON P.COD_CARGA = C.COD_CARGA
                                INNER JOIN PEDIDO_PRODUTO PP ON PP.COD_PEDIDO = P.COD_PEDIDO
                                WHERE E.COD_EXPEDICAO = $idExpedicao
                                GROUP BY E.COD_EXPEDICAO, C.COD_CARGA, PP.COD_PRODUTO, PP.DSC_GRADE, P.SEQUENCIA, C.DSC_PLACA_CARGA, P.COD_PESSOA, P.COD_PEDIDO
                                ) PP ON PP.COD_EXPEDICAO = E.COD_EXPEDICAO AND PP.COD_PRODUTO = CONF.COD_PRODUTO AND PP.DSC_GRADE = CONF.DSC_GRADE
                    INNER JOIN CARGA C ON PP.COD_CARGA = C.COD_CARGA
                    INNER JOIN PRODUTO PROD ON PROD.COD_PRODUTO = CONF.COD_PRODUTO AND PROD.DSC_GRADE = CONF.DSC_GRADE
                    INNER JOIN LINHA_SEPARACAO LS ON PROD.COD_LINHA_SEPARACAO = LS.COD_LINHA_SEPARACAO
    ...
    {
        $sql = "SELECT MS.COD_MAPA_SEPARACAO, MSPROD.COD_PRODUTO, MSPROD.DSC_GRADE, MSPROD.COD_PRODUTO_VOLUME, MSPROD.COD_PRODUTO_EMBALAGEM, MSPROD.QTD_EMBALAGEM, ((NVL(MSPROD.QTD_SEPARAR * MSPROD.QTD_EMBALAGEM,0) - (NVL(MSC.QTD_CONFERIDA,0) * NVL(MSC.QTD_EMBALAGEM,0)) - NVL(PP.QTD_CORTADA,0)) / MSPROD.QTD_EMBALAGEM) QTD_CONFERIDA, NULL, 1 NUM_CONFERENCIA, 'N' IND_CONFERENCIA_FECHADA, SYSDATE, NULL COD_VOLUME_PATRIMONIO, NULL COD_MAPA_SEPARACAO_EMBALADO, NULL COD_PESSOA
                    FROM EXPEDICAO E
                    INNER JOIN CARGA C ON E.COD_EXPEDICAO = C.COD_EXPEDICAO
                    INNER JOIN PEDIDO P ON P.COD_CARGA = C.COD_CARGA
                    INNER JOIN PEDIDO_PRODUTO PP ON PP.COD_PEDIDO = P.COD_PEDIDO
                    ...
                    WHERE E.COD_EXPEDICAO = $idExpedicao AND ((NVL(MSPROD.QTD_SEPARAR * MSPROD.QTD_EMBALAGEM,0) - (NVL(MSC.QTD_CONFERIDA,0) * NVL(MSC.QTD_EMBALAGEM,0)) - NVL(PP.QTD_CORTADA,0)) / MSPROD.QTD_EMBALAGEM) > 0" ;
        return $this->getEntityManager()->getConnection()->query($sql)->fetchAll(\PDO::FETCH_ASSOC);
    }
