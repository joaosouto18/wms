...
class VSaldoLoteCompletoRepository extends EntityRepository implements VSaldoInterface
{
    ...
    public function saldo($params)
    {
        ...
        ->select("s.codProduto, s.grade,s.dscLinhaSeparacao, s.qtd, p.descricao, s.dscEndereco, MOD(e.predio,2) as lado, NVL(s.lote, ' ') lote, e.id as idEndereco, s.codUnitizador, s.unitizador, s.volume, tp.descricao as tipoComercializacao, MAX(pe.quantidade) quantidade")
        ->from("wms:Enderecamento\VSaldoLoteCompleto","s")
        ...
        ->groupBy("s.codProduto, s.grade, s.dscLinhaSeparacao, s.qtd, s.lote, p.descricao, s.dscEndereco, e.predio, e.id, s.codUnitizador, s.unitizador, s.volume, tp.descricao, e.rua, e.predio, e.nivel, e.apartamento, s.codProduto, s.grade, s.volume")
        ...
        if (!empty($params['lote'])) {
            $query->andWhere("s.lote = '$params[lote]'");
        }
