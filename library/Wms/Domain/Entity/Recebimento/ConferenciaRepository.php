...
class ConferenciaRepository extends EntityRepository
{
    ...
    public function getQtdByRecebimentoVolumeAndNorma ($idOs, $codProduto, $grade){
        $SQL = "SELECT MIN (QTD) as QTD, COD_NORMA_PALETIZACAO, NUM_NORMA, COD_UNITIZADOR, SUM(NUM_PESO) as PESO, LOTE
                  FROM (SELECT SUM(QTD_CONFERIDA) as QTD, RV.COD_PRODUTO_VOLUME, RV.COD_NORMA_PALETIZACAO, NP.NUM_NORMA, NP.COD_UNITIZADOR, SUM(RV.NUM_PESO) as NUM_PESO, NVL(RV.DSC_LOTE, '') AS LOTE
                         ...
                         GROUP BY RV.COD_PRODUTO_VOLUME, RV.COD_NORMA_PALETIZACAO, NP.NUM_NORMA, COD_UNITIZADOR, NVL(RV.DSC_LOTE, ''))
    ...
    public function getQtdByRecebimentoEmbalagemAndNorma ($idOs, $codProduto, $grade){
        $SQL = "SELECT MAX(RE.COD_RECEBIMENTO_EMBALAGEM) ORDENATOR, SUM(RE.QTD_CONFERIDA * RE.QTD_EMBALAGEM) as QTD, 0 as COD_NORMA_PALETIZACAO, (NP.NUM_NORMA * PENP.QTD_EMBALAGEM) as NUM_NORMA, NP.COD_UNITIZADOR, SUM(RE.NUM_PESO) as PESO, NVL(RE.DSC_LOTE, '') AS LOTE
                  FROM RECEBIMENTO_EMBALAGEM RE
                 INNER JOIN PRODUTO_EMBALAGEM PE ON PE.COD_PRODUTO_EMBALAGEM = RE.COD_PRODUTO_EMBALAGEM
                 INNER JOIN NORMA_PALETIZACAO NP ON NP.COD_NORMA_PALETIZACAO = RE.COD_NORMA_PALETIZACAO  
                 INNER JOIN PRODUTO_DADO_LOGISTICO PDL ON PDL.COD_NORMA_PALETIZACAO = NP.COD_NORMA_PALETIZACAO
                 INNER JOIN PRODUTO_EMBALAGEM PENP ON PENP.COD_PRODUTO_EMBALAGEM = PDL.COD_PRODUTO_EMBALAGEM
                 WHERE COD_OS = '$idOs'
                   AND PE.COD_PRODUTO = '$codProduto'
                   AND PE.DSC_GRADE = '$grade'
                 GROUP BY  (NP.NUM_NORMA * PENP.QTD_EMBALAGEM) , NP.COD_UNITIZADOR, NVL(RE.DSC_LOTE, '')
                 ORDER BY ORDENATOR
          ";
