...
class Filter
{
     ...
     *
     * @param \Column $column
     * @return \Core\Grid\Column\Filter 
     */
     ...
     * Grid\Column\Filter\Render. 
     *
     * @param  array|Grid\Column\Filter\Render\ARender $render
     * @return \Core\Grid\Column\Filter
     * @throws \Exception 
     */
	...
	if (is_array($render)) {
		...
		$className = "\Core\Grid\Column\Filter\Render\\{$type}";
	...
	} elseif (is_object($render) && $render instanceof \Core\Grid\Column\Filter\Render\IRender) {
	    $render = $render;
	} elseif (is_string($render)) {
	    $render = ucfirst($render);
	    $className = "\Core\Grid\Column\Filter\Render\\{$render}";
