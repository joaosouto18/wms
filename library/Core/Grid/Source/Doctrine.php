...
class Doctrine extends \Core\Grid\Source\ASource implements \Core\Grid\Source\ISource
{
    ...
    public function setEntityManager($entityManager = null)
    {
        if(is_null($entityManager)) {
            if(\Zend_Registry::isRegistered('doctrine')) {
                ...
                throw new \Core\Grid\Source\Doctrine2\Exception('No suitable EntityManager found in registry, please set a specific one.');
            }
        } elseif(!($entityManager instanceof EntityManager)) {
            throw new \Core\Grid\Source\Doctrine2\Exception('Parameter must be an instance of \Doctrine\ORM\EntityManager');
        }
    ...
    public function setQueryBuilder($value)
    {
        ...
        if(is_string($value)) {
            ...
            if(!@class_exists($value, true)) {
                throw new \Core\Grid\Source\Doctrine2\Exception('Entity with name ' . $value . ' does not exist.');
            }
            ...
            throw new \Core\Grid\Source\Doctrine2\Exception('Parameter must be an instance of \Doctrine\ORM\EntityRepository or \Doctrine\ORM\QueryBuilder.');
        } else {
            $qb = $value;
        }
    ...
    {
        throw new \Core\Grid\Source\Doctrine2\Exception("Fulltext searching is currently not supported by the Doctrine2 source.");
    }
                ...
                foreach($assoc as $field) {
                    if(is_object($field)) {
                        ...
                        if(!method_exists($field, $method)) {
                            throw new \Core\Grid\Source\Doctrine2\Exception('No getter method for the primary field found (used name: ' . $method . ').');
                        }
    ...
    {
        throw new \Core\Grid\Source\Doctrine2\Exception('Not yet implemented.');
    }
    ...
    private function _getModelFromColumn($column)
    {
        if(!is_string($column)) {
            ...
            throw new \Core\Grid\Source\Doctrine2\Exception('The $column param needs to be a string, ' . $type . ' provided');
        }
    ...
    private function _getModelFromAlias($alias)
    {
        ...
        if(is_null($field)) {
            throw new \Core\Grid\Source\Doctrine2\Exception("No field found.");
        }
        ...
        throw new \Core\Grid\Source\Doctrine2\Exception("No model found.");
    }
